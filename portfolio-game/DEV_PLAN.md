...
- [x] Define tile map data structure — `zone.h`: `Tile` struct, `TileMap` struct, load/draw/collision functions (tiles 0–5: air, ground, water surface, deep water, rock, platform)
- [x] Draw Beach zone tilemap — `zones/beach.h`: 240×45 tiles (3840px wide), ocean, flat sand, tidal pool (cols 95–145), elevated platform (cols 180–225)
- [x] Replace runtime terrain gen with tilemap renderer in SDL2 — `main.cpp` refactored, procedural heightmap removed
- [x] Verify camera scroll across full zone width — zone constants corrected (Beach 0–3840, Forest 3840–7680, Wetlands 7680–11520), camera debug bar added to HUD

### 0.2 — Art pipeline setup
- [x] Agree on sprite sheet format — 16×16 tiles, PNG with transparency
- [x] Write SDL2 sprite sheet loader — `spritesheet.h` + `spritesheet.cpp`: `loadSpriteSheet()`, `drawFrame()`, `destroySpriteSheet()`, null-safe, SDL2_image integrated
- [x] Write animation system — `animation.h`: `AnimationClip`, `AnimationController`, `addClip()`, `playClip()`, `updateAnimation()`, `registerPlayerClips()`
- [x] Test with placeholder 16×16 player sprite — `createPlaceholderPlayerSheet()`: 256×32 texture, 16 coloured frames, clips: idle/walk/jump/pickup auto-switch on state

### 0.3 — Code architecture refactor
- [x] Split `main.cpp` into modules — `constants.h`, `types.h`, `helpers.h/cpp`, `world.h/cpp`, `player.h/cpp`, `hud.h/cpp`, `entity.h/cpp`, `game.h/cpp`. `main.cpp` trimmed to 56 lines. Makefile lists all 8 `.cpp` compile units.
